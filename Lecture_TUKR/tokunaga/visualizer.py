...
STEP = 150
def visualize_real_history(data, U_history, V_history, error_history, save_gif, filename):
    #input_dim = 1
    latent_dim1=U_history[0].shape[1]
    latent_dim2=V_history[0].shape[1]
    ...
    latent2_ax = fig.add_subplot(gs[0:2, 1], aspect='equal')
    error_ax = fig.add_subplot(gs[2, :])
    num_epoch = len(U_history)
    ...
    ani = FuncAnimation(
        ...
        repeat=True,
        interval=50,
        fargs=(latent1_drawer, latent2_drawer,  U_history, V_history, error_history, fig,
               latent1_ax, latent2_ax, error_ax, num_epoch, data))
    plt.show()
    if save_gif:
...
def visualize_history(X, Y_history, U_history, V_history, error_history, save_gif=False, filename="tmp"):
    ...
    ani = FuncAnimation(
        ...
        repeat=True,
        interval=50,
        fargs=(observable_drawer, latent1_drawer, latent2_drawer, X, U_history, V_history, error_history, fig,
               input_ax, latent1_ax, latent2_ax, error_ax, num_epoch))
    plt.show()
    if save_gif:
        ani.save(f"{filename}.mp4", writer='ffmpeg')

def update_real_graph(epoch, latent1_drawer, latent2_drawer, U_history, V_history, error_history, fig, latent1_ax, latent2_ax, error_ax, num_epoch, data):
    ...
    U = U_history[epoch]
    V = V_history[epoch]
    #colormapx = np.arange(X.shape[0]*X.shape[1])
    colormapu=np.arange(U.shape[0])
    colormapv=np.arange(V.shape[0])
    #input_drawer(input_ax, X, Y, colormapx)
    latent1_drawer(latent1_ax, U, colormapu, data, 1)
    latent2_drawer(latent2_ax, V, colormapv, data, 2)
...
def draw_latent_2D(ax, Z, colormap, data, n):
    # Z_max=np.amax(Z, axis=0)
    # Z_min=np.amin(Z, axis=0)
    # ax.set_xlim(Z_min[0] - 0.1, Z_max[0] + 0.1)
    # ax.set_ylim(Z_min[1] - 0.1, Z_max[1] + 0.1)
    ax.set_xlim(-1, 1)
    ax.set_ylim(-1, 1)
    ax.scatter(Z[:, 0], Z[:, 1], c=colormap)
    for (i) in range(Z.shape[0]):
        ax.annotate(data[n][i], xy=(Z[i, 0], Z[i, 1]))


def draw_latent_1D(ax, Z, colormap, data, n):
    ax.scatter(Z, np.zeros(Z.shape), c=colormap)
    #ax.set_title('a')
    ax.set_ylim(-1, 1)
    for (i) in range(Z.shape[0]):
        ax.annotate(data[n][i], xy=(Z[i, 0], np.zeros(Z.shape)[i, 0]))
